...
def _run_tracking(runtime: RuntimeConfig, camera: CameraConfig, cache_path: Path, tracker: UltralyticsByteTracker | None) -> list[dict[str, Any]]:
...
def run_pipeline(
    ...
    *,
    ...
    if mode == "full_ai":
        frames = _run_tracking(runtime, camera, default_cache, tracker)
